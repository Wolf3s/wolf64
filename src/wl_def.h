...
#define ABS(x)          ({ typeof(x) _x = (x); (int)_x > 0 ? _x : -_x; })
#define LABS(x)         ({ typeof(x) _x = (x); (int32_t)_x > 0 ? _x : -_x; })
...
extern  int8_t      buttonmouse[2];
extern  int8_t      buttonjoy[16];
extern  uint8_t     MoveMode;
extern  boolean     autorun;

extern  uint32_t    buttonheld;
...
extern  int         mouseturn;              // range from -100 to 100
...
#define BUTTONSTATE(_bt) (buttonstate & (1<<(_bt)))
#define BUTTONHELD(_bt) (buttonheld & (1<<(_bt)))
#define BUTTONPRESSED(_bt) (BUTTONSTATE(_bt) && !BUTTONHELD(_bt))
#define SETBUTTONSTATE(_bt) (buttonstate |= (1<<(_bt)))
#define SETBUTTONHELD(_bt) (buttonheld |= (1<<(_bt)))
#define UNSETBUTTONSTATE(_bt) (buttonstate &= ~(1<<(_bt)))
#define UNSETBUTTONHELD(_bt) (buttonheld &= ~(1<<(_bt)))
