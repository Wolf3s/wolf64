...
byte GetColor(byte red, byte green, byte blue, uint16_t *palette)
{
    int col;
    byte mincol = 0;
    double mindist = 200000.F, curdist, DRed, DGreen, DBlue;

    uint16_t *palPtr = palette;

    for(col = 0; col < 256; col++, palPtr++)
    {
        color_t c = color_from_packed16(*palPtr);
        DRed   = (double) (red   - c.r);
        DGreen = (double) (green - c.g);
        DBlue  = (double) (blue  - c.b);
