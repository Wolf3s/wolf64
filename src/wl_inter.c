...
PreloadUpdate (unsigned current, unsigned total)
{
    ...
    VWB_BarScaledCoord (WindowX +  5, WindowY + WindowH -  3,
        w,  2, BLACK);
    w = ((int32_t) w * current) / total;
    if (w)
    {
        VWB_BarScaledCoord (WindowX +  5, WindowY + WindowH -  3,
            w,  2, 0x37);       //SECONDCOLOR);
        VWB_BarScaledCoord (WindowX +  5, WindowY + WindowH -  3,
            w -  1,  1, 0x32);
...
PreloadGraphics (void)
{
    ...
    VWB_DrawPicScaledCoord (((screenWidth-224)/16) * 8,
        (screenHeight-(STATUSLINES+48))/2, GETPSYCHEDPIC);
