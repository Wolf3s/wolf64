...
void DrawPlayBorderSides(void)
{
    ...
    if(xl != 0)
    {
	    VWB_BarScaledCoord(0,            0, xl - 1,     h, bordercol);                 // left side
	    VWB_BarScaledCoord(xl + vw + 1, 0, xl - 1 * 2, h, bordercol);                 // right side
    }

    if(yl != 0)
    {
	    VWB_BarScaledCoord(0, 0,            sw, yl - 1, bordercol);                    // upper side
	    VWB_BarScaledCoord(0, yl + vh + 1, sw, yl - 1, bordercol);                    // lower side
    }

    if(xl != 0)
    {
        // Paint game view border lines
	    VWB_BarScaledCoord(xl - 1, yl - 1, vw + 1, 1,          0);                      // upper border
	    VWB_BarScaledCoord(xl,      yl + vh, vw + 1, 1,          bordercol - 2);          // lower border
	    VWB_BarScaledCoord(xl - 1, yl - 1, 1,      vh + 1,     0);                      // left border
	    VWB_BarScaledCoord(xl + vw, yl - 1, 1,      vh + 1 * 2, bordercol - 2);          // right border
	    VWB_BarScaledCoord(xl - 1, yl + vh, 1,      1,          bordercol - 3);          // lower left highlight
    }
    else
    {
        // Just paint a lower border line
        VWB_BarScaledCoord(0, yl+vh, vw, 1, bordercol-2);       // lower border
    }
}
...
void DrawStatusBorder (byte color)
{
    ...
    VWB_BarScaledCoord (0,0,screenWidth,screenHeight-(STATUSLINES-3),color);
    VWB_BarScaledCoord (0,screenHeight-(STATUSLINES-3),
        statusborderw+8,(STATUSLINES-4),color);
    VWB_BarScaledCoord (0,screenHeight-2,screenWidth,2,color);
    VWB_BarScaledCoord (screenWidth-statusborderw-8, screenHeight-(STATUSLINES-3),
        statusborderw+8,(STATUSLINES-4),color);

    VWB_BarScaledCoord (statusborderw+9, screenHeight-3,
        97, 1, color-1);
    VWB_BarScaledCoord (statusborderw+106, screenHeight-3,
        161, 1, color-2);
    VWB_BarScaledCoord (statusborderw+267, screenHeight-3,
        44, 1, color-3);
    VWB_BarScaledCoord (screenWidth-statusborderw-9, screenHeight-(STATUSLINES-4),
        1, 20, color-2);
    VWB_BarScaledCoord (screenWidth-statusborderw-9, screenHeight-(STATUSLINES/2-4),
        1, 14, color-3);
}
...
void DrawPlayBorder (void)
{
    ...
    {
        const int statusborderw = (screenWidth-320)/2;
        VWB_BarScaledCoord (0, screenHeight-STATUSLINES,
            statusborderw+8, STATUSLINES, bordercol);
        VWB_BarScaledCoord (screenWidth-statusborderw-8, screenHeight-STATUSLINES,
            statusborderw+8, STATUSLINES, bordercol);
    }

    if((unsigned) viewheight == screenHeight) return;

    VWB_BarScaledCoord (0,0,screenWidth,screenHeight-STATUSLINES,bordercol);

    const int xl = screenWidth/2-viewwidth/2;
    const int yl = (screenHeight-STATUSLINES-viewheight)/2;
    VWB_BarScaledCoord (xl,yl,viewwidth,viewheight,0);

    if(xl != 0)
    {
        // Paint game view border lines
        VWB_BarScaledCoord(xl-1, yl-1, viewwidth+1, 1, 0);                      // upper border
        VWB_BarScaledCoord(xl, yl+viewheight, viewwidth+1, 1, bordercol-2);       // lower border
        VWB_BarScaledCoord(xl-1, yl-1, 1, viewheight+1, 0);                     // left border
        VWB_BarScaledCoord(xl+viewwidth, yl-1, 1, viewheight+2*1, bordercol-2);  // right border
        VWB_BarScaledCoord(xl-1, yl+viewheight, 1, 1, bordercol-3);              // lower left highlight
    }
    else
    {
        // Just paint a lower border line
        VWB_BarScaledCoord(0, yl+viewheight, viewwidth, 1, bordercol-2);       // lower border
    }
}
...
void DrawPlayScreen (void)
{
    VWB_DrawPicScaledCoord ((screenWidth-320)/2,screenHeight-STATUSLINES,STATUSBARPIC);
    DrawPlayBorder ();
...
void Died (void)
{
    ...
    surface_t *fadeSurf = VL_GrabScreen();
    VL_BarScaledCoord (viewscreenx,viewscreeny,viewwidth,viewheight,4);

    FizzleFade(screenBuffer,fadeSurf,viewscreenx,viewscreeny,viewwidth,viewheight,70,false);
