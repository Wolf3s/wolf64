...
void ControlMovement (objtype *ob)
{
    ...
    strafing = !!MoveMode != !!BUTTONSTATE(bt_strafe);
    delta = (autorun != !!BUTTONSTATE(bt_run)) ? RUNMOVE * tics : BASEMOVE * tics;
    ...
    if(strafing ? BUTTONSTATE(bt_turnleft) : BUTTONSTATE(bt_strafeleft))
    {
        angle = ob->angle + ANGLES/4;
        if(angle >= ANGLES)
            angle -= ANGLES;
        Thrust(angle, delta * MOVESCALE);
    }

    if(strafing ? BUTTONSTATE(bt_turnright) : BUTTONSTATE(bt_straferight))
    {
        angle = ob->angle - ANGLES/4;
        if(angle < 0)
            angle += ANGLES;
    ...
    if(strafing ? BUTTONSTATE(bt_straferight) : BUTTONSTATE(bt_turnright))
    {
        turnamount += delta;
    }
    if(strafing ? BUTTONSTATE(bt_strafeleft) : BUTTONSTATE(bt_turnleft))
    {
        turnamount -= delta;
    }
    ...
    {
        Thrust (ob->angle,delta*MOVESCALE); // move forwards
    }
    if (BUTTONSTATE(bt_movebackward))
    {
        ...
        if (angle >= ANGLES)
            angle -= ANGLES;
        Thrust (angle,delta*BACKMOVESCALE);          // move backwards
    }
    if (controly < 0)
    {
...
{
    VWB_DrawPicScaledCoord (((screenWidth-320)/16 + x) * 8,
        screenHeight-(STATUSLINES-y),picnum);
}
...
void    T_Attack (objtype *ob)
{
    ...
    if ( BUTTONSTATE(bt_use) && !BUTTONHELD(bt_use) )
        UNSETBUTTONSTATE(bt_use);

    if ( BUTTONSTATE(bt_attack) && !BUTTONHELD(bt_attack))
        UNSETBUTTONSTATE(bt_attack);
